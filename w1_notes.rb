...
i.e:
	...
	ARRAY		[“carrots”,”apples”,”oranges”].each do |item|
			puts item
			end

				-------------------------

	HASH		{name: "John", surname: "Doe"}.each do |key,value|
			puts "#{key} -> #{value}"
			end

	WITH 	dict = {}
	OBJECT		[["a",1],["b",2],["c",3]].each do |item|
				key = item[0].to_sym
				dict[key] = item[1]
			end
			dict
	...
	WITH    i = 0
	INDEX	["a","b","c"].each do |x|
				puts "[#{i}] #{x}"
				i += 1
			end

			SHORTHAND: ["a","b","c"].each_with_index {|x,i| puts "[#{i}] #{x}"}
